...
vector<double> laguerre::laguerre(vector<double> y, vector<vector<double>> x, int n, int k, int method){
...
//    }
    vector<double> b(k);
    vector<double> row(k);
    vector<vector<double>> A(k, row);
    double k_1, k_2, k_3 = 0, k_4 = 0;
    for (int i = 0; i < 100000; ++i){
        k_1 = k1(x[i][n], method);
        k_2 = k2(x[i][n], method);
    ...
    //std::cout << n << std::endl;
    vector<double> a = matrix_cal::linear_equation(A,b,k);
    ...
    for (int i = 0; i < 100000; ++i){
        ...
        result.push_back(a[0] * k1(x[i][n], method) + a[1] * k2(x[i][n], method));
        if (k > 2){
            result[i] += a[2] * k3(x[i][n], method);
        }
        if (k > 3){
