    ...
    for (int i = size - 2; i > 0; --i){
        for (int j = 0; j < path_count; ++j){
            if (index[j] < size){
                ...
                test[j] = exp(-r * delta * temp) * (strike - path[j][index[j]]);
            }
            else{
                test[j] = 0;
            }
...
//    }
    ...
    for (int i = 0; i < path_count; ++i){
        if (index[i] < size){
            sum += exp(-r * delta * index[i]) * (strike - path[i][index[i]]);
            if (strike - path[i][index[i]] < 0){
                std::cout << "(x - path[i][index[i]]): " << (strike - path[i][index[i]]) << std::endl;
            }
        }
